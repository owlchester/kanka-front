<template>
  ...
  <div v-else>
    ...
    <Section>
      <div>
      <NuxtLink to="/votes" class="btn-round rounded-full">See all votes</NuxtLink>
      </div>
    </Section>
  </div>
</template>
...
useHead({
  title: vote.value.data.name + ' - Kanka',
  meta: [
    { name: 'description', content: vote.value.data.excerpt }
  ],
})
